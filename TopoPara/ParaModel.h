...
class ParaTreeModel : public QAbstractItemModel
{
    Q_OBJECT
public:
    explicit ParaTreeModel(QObject *parent = nullptr);
    ~ParaTreeModel();

    void setRootItem(tdsTopoPara *root, QStringList header);
...
//public:
    tdsPara *   itemFromIndex(const QModelIndex &index) const;
    tdsTopoPara *itemRoot() const;

    void clear();

private:
    QStringList mHeaders;   //表头内容
    tdsTopoPara *mRootItem;    //根节点

    int         numWithKeyFromParent(const QModelIndex &index) const;
};

class ParaTableModel : public QAbstractItemModel
{
    Q_OBJECT
public:
    explicit ParaTableModel(QObject *parent = nullptr);
    ~ParaTableModel();

    void setRootItem(tdsPara *root, QStringList header);
    //以下为自定义model需要实现的一些虚函数，将会被Qt在查询model数据时调用
    //headerData: 获取表头第section列的数据
    //data: 核心函数，获取某个索引index的元素的各种数据
    //      role决定获取哪种数据，常用有下面几种：
    ...
    bool setData(const QModelIndex &index, const QVariant &value,
                 int role = Qt::EditRole) override;
//public:
    tdsItem *   itemFromIndex(const QModelIndex &index) const;
    tdsPara *   itemRoot() const;

    void clear();
private:
    QStringList     mHeaders;   //表头内容
    tdsPara *       mRootItem;    //根节点
};
