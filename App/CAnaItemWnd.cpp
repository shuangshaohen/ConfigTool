...
void CAnaItemWnd::itemDoubleClickedSlot(QTableWidgetItem *item)
{
    if(item->column() == Enum_PubTable_Attr_Col)
    {
        AnaItem * p = (AnaItem *)item->data(Qt::UserRole+1).toULongLong();
        QStringList list;
...
bool CAnaItemWnd::checkItemVal(QTableWidgetItem *item)
{
    ...
    case Enum_AnaTable_RateSetP_Col:
        ok = m_data->checkSPSetCnnInfo(p->sRateSetP);
        break;
    case Enum_AnaTable_RateSetS_Col:
        ok = m_data->checkSPSetCnnInfo(p->sRateSetS);
        break;
    case Enum_AnaTable_RecSYb_Col:
        ok = m_data->checkSoftYBCnnInfo(p->sRecSYb);
        break;
    case Enum_AnaTable_RecHYb_Col:
        ok = m_data->checkHardBICnnInfo(p->sRecHYb);
        break;
...
void CAnaItemWnd::writeConfigVal(QTableWidgetItem *item)
{
    ...
    case Enum_AnaTable_ChanFlag_Col:
        p->sChanFlag = item->text();
        break;
    case Enum_AnaTable_XuYCDft_Col:
        p->iXuYCDft = item->text().toInt();
        break;
    case Enum_AnaTable_Coe_Col:
        p->wCoe = item->text().toUInt();
        break;
    case Enum_AnaTable_RateSetP_Col:
        if(m_data->checkSPSetCnnInfo(item->text()) == false)
        {
            CMsgInfo msgInfo( CMsgInfo::Enum_Application_Verify_Mode, CMsgInfo::CN_ERROR_MSG,
                              ...
                              .arg(item->row()+1).arg(p->sRateSetP).arg(item->text()));
            outPutMsgInfo(msgInfo);
        }
        p->sRateSetP = item->text();
        break;
    case Enum_AnaTable_RateSetS_Col:
        if(m_data->checkSPSetCnnInfo(item->text()) == false)
        {
            CMsgInfo msgInfo( CMsgInfo::Enum_Application_Verify_Mode, CMsgInfo::CN_ERROR_MSG,
                              ...
                              .arg(item->row()+1).arg(p->sRateSetS).arg(item->text()));
            outPutMsgInfo(msgInfo);
        }
        p->sRateSetS = item->text();
        break;
    ...
    case Enum_AnaTable_RecSYb_Col:
        if(m_data->checkSoftYBCnnInfo(item->text()) == false)
        {
            CMsgInfo msgInfo( CMsgInfo::Enum_Application_Verify_Mode, CMsgInfo::CN_ERROR_MSG,
                              ...
                              .arg(item->row()+1).arg(p->sRecSYb).arg(item->text()));
            outPutMsgInfo(msgInfo);
        }
        p->sRecSYb = item->text();
        break;
    case Enum_AnaTable_RecHYb_Col:
        if(m_data->checkHardBICnnInfo(item->text()) == false)
        {
            CMsgInfo msgInfo( CMsgInfo::Enum_Application_Verify_Mode, CMsgInfo::CN_ERROR_MSG,
                              ...
                              .arg(item->row()+1).arg(p->sRecHYb).arg(item->text()));
            outPutMsgInfo(msgInfo);
        }
        p->sRecHYb = item->text();
        break;
