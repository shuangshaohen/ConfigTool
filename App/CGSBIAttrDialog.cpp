...
#include "CDataFun.h"
#include "ui_GSBIAttrDialog.h"

CGSBIAttrDialog::CGSBIAttrDialog(QWidget *parent, QTableWidgetItem *pItem) :
    QDialog(parent),
    ui(new Ui::CGSBIAttrDialog),
    m_pItem(pItem)
{
    ui->setupUi(this);
    ...
    m_pGSBI = (GooseBiConfig*)(pItem->data( Qt::UserRole ).toUInt());

    CreateCheckBoxes();

    if(m_pGSBI != NULL)
    {
        setWindowTitle(m_pGSBI->sName + "属性设置");
        SetCheckBox(m_pGSBI->dwAttr);
    }

    connect(ui->buttonBox,SIGNAL(accepted()),this,SLOT(DialogAcceptSlot()));
}

CGSBIAttrDialog::~CGSBIAttrDialog()
{
    delete ui;
    m_CheckBoxes.clear();
    m_pGSBI = NULL;
...
void CGSBIAttrDialog::CreateCheckBoxes()
{
    m_CheckBoxes.clear();
...
void CGSBIAttrDialog::SetCheckBox(unsigned int dwAttr)
{
...
void CGSBIAttrDialog::DialogAcceptSlot()
{
    //m_pGSBI->dwAttr = 0;
